...
impl RTCStatsAccumulator {
    ...
    pub fn snapshot(&self, now: Instant) -> RTCStatsReport {
        ...
        for (id, acc) in &self.data_channels {
            reports.insert(format!("RTCDataChannel_{}", id), RTCStatsReportType::DataChannel(acc.snapshot(now)));
        }
...
| `local_certificate_id`            | String                | ✅         | DTLS             | dtls.rs:114           | From cert fingerprint                  |
...
| `transport.local_certificate_id`             | ✅      | `update_dtls_stats_from_profile` | From cert fingerprint       |
